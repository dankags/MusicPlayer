  ...
  return (
    ...
    <div className='overflow-y-scroll  w-full h-full relative rounded-md scrollbar-thin scrollbar-thumb-neutral-700 bg-neutral-900 '>
      <div className={`w-full mb-4`} style={{backgroundImage:`linear-gradient(0deg,#171717,${containerColor})`}}>
        ...
        <span className='text-3xl font-semibold px-4 pt-12'>Good afternoon</span>
          <div className={`w-full px-3 mt-3 grid grid-flow-row gap-2  ${pageWidth<=642 ? "grid-cols-2 max-md:grid-cols-1  " : "grid-cols-3 md:grid-cols-3 sm:grid-cols-2 max-sm:grid-cols-1"}`} >
    ...
    <div className='pt-4'>
      <div className='flex items-center justify-between px-5'>
        ...
        <UserPlayListHandler  visibleItemCount={3} otherClassNames={""}>   
          <UserPlayList/>
          <UserPlayList/>
          <UserPlayList/>
          <UserPlayList/>
...
const UserRecomended = ({ pageWidth, data, setContainerColor }) => {
  ...
  return (
    <div onMouseOut={()=>setContainerColor("#1e3a8a")} onMouseOver={()=>setContainerColor(color)} className={`featuredMusics max-h-44  cursor-pointer flex items-center rounded-md gap-2 transition-all  ease-in-out  dark:bg-neutral-500/40 hover:duration-300 hover:bg-neutral-500/75 ${pageWidth<=642?'col-span-1 ':'col-span-1'}`}>
            <div className='w-4/12 h-20 relative flex justify-start'>
              <Image src={data.img} alt='' width={80} height={100} className='object-cover rounded-s-md'/> 
            </div>
...
const UserPlayListHandler = ({children,otherClassNames,visibleItemCount=Number.POSITIVE_INFINITY}) => {
  const childrenArray = React.Children.toArray().flat()
  const visibleChildren = childrenArray.slice(0, visibleItemCount)
  children=visibleChildren
  return (
    <div className={`${twMerge("px-5 flex justify-between gap-2 mt-4",otherClassNames)}`} >
      {children}
     </div>
   )
}
